...
description: React hooks available in Better Tables UI package
---
...
- **Export**: `useExport` for data export with progress tracking
...
A comprehensive hook for managing data export operations with progress tracking, cancellation, and automatic file downloads.

### Signature

```typescript
function useExport<TData = unknown>(options: UseExportOptions<TData>): UseExportResult;
```

### Parameters

```typescript
interface UseExportOptions<TData = unknown> {
  /** Column definitions for export */
  columns: ColumnDefinition<TData>[];
  /** Table adapter for fetching data */
  adapter: TableAdapter<TData>;
  /** Current filters to apply during export */
  filters?: FilterState[];
  /** Current sorting to apply during export */
  sorting?: SortingParams[];
  /** Custom value transformer for formatting exported values */
  valueTransformer?: (value: unknown, row: TData, column: ColumnDefinition<TData>) => unknown;
  ...
  /** Whether to automatically download the file (default: true) */
...
function ExportSection({ columns, adapter }) {
  const {
    startExport,
    cancelExport,
    progress,
...
### Advanced Usage with Custom Value Transformer
...
const { startExport } = useExport({
  ...
  valueTransformer: (value, row, column) => {
    ...
    // Format currency
    if (column.type === 'number' && column.id.includes('price')) {
      return `$${Number(value).toFixed(2)}`;
    }
    return value;
  },
  onComplete: (result) => {
    console.log(`Export complete: ${result.rowCount} rows, ${result.fileSize} bytes`);
  },
});
```
...
The `useExport` hook is integrated into the BetterTable component. Simply provide the `export` prop:
