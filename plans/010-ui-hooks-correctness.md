...
- `packages/ui/src/hooks/use-table-url-sync.ts` — three defects:
  ...
  3. Re-subscribe churn: both effects depend on `config` (`:84`, `:221`), and consumers pass inline objects — `packages/ui/src/components/table/table.tsx:275` builds `urlSync?.config || {…}` inline; `apps/marketing/src/components/sections/users-table-client.tsx:31-40` passes an inline literal — so every render tears down/recreates the subscription and a fresh debounce closure (multiplying leak #1).
