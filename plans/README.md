...
| [050](050-export-ui.md) | Export UI: `ExportButton`/`useTableExport` + `csvExport()` plugin + row-cap decision | **049 (DONE)**, **059 (DONE)** | TODO — **now fully unblocked** (reconciled 2026-07-18; **2026-07-21: `plans/design/ui-modules.md` now EXISTS (059 landed); the plugin hook seam (049) landed — `csvExport()` builds on `beforeFetch`/`afterFetch` + the `[key:string]` capability escape hatch — refresh 050 against it before executing. `ExportButton` rides 059's `toolbarExtra` slot (props: `columns`/`adapter`/`filters`/`sorting`/`totalCount` — validated against 050's needs in the design record) and ships as an `export` module; `TableConfig.exportOptions` was removed by 057, reintroduce only what the module needs as module-local props**) |
