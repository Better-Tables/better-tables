...
| [049](049-plugin-hook-execution.md) | Execute the plugin hook seam (`beforeFetch`/`afterFetch`), validated by one real plugin | 018 (done) | DONE (2026-07-21) — `TableDefPlugin` models `beforeFetch`/`afterFetch`; `tables.fetchData` runs them in array order (empty-loop fast path; throwing hook propagates); `logPlugin()` shipped in `packages/core/src/plugins/`; 7 hook tests; architecture doc + design-doc §5 updated; `.changeset/plugin-fetch-hooks.md` (core minor). Additional hook points deferred per the minimal-hooks rule |
