...
| [049](049-plugin-hook-execution.md) | Execute the plugin hook seam (`beforeFetch`/`afterFetch`), validated by one real plugin | 018 (done) | DONE (2026-07-21) — `TableDefPlugin` models `beforeFetch`/`afterFetch`; `tables.fetchData` runs them in array order (empty-loop fast path; throwing hook propagates); `logPlugin()` shipped in `packages/core/src/plugins/`; 8 hook tests (incl. primaryTable stays pinned across a `beforeFetch` that returns a bare object); architecture doc (new Plugins section) + design-doc §5 updated; `.changeset/plugin-fetch-hooks.md` (core minor). Additional hook points deferred per the minimal-hooks rule. **Migration note**: the plan listed `MIGRATION.md`, but that file is not tracked on `main` (only in agent worktrees), so the "hooks now execute" guidance landed in the changeset + `docs/architecture` Plugins section instead — no `MIGRATION.md` edit was possible/needed |
| [050](050-export-ui.md) | Export UI: `ExportButton`/`useTableExport` + `csvExport()` plugin + row-cap decision | **049 (DONE)**, **059 (DONE)** | DONE (2026-07-21) — row cap = `DEFAULT_EXPORT_ROW_CAP` (50k, `maxRows` override); `ExportParams` gained `filters`/`sorting`/`maxRows` (additive — export now honors the current view; deviates from the plan's "no signature change" by design, see below); drizzle `exportData` honors filters/sorting + cap; `httpAdapter` gained `exportData` (reuses fetch proxy, formats CSV/JSON client-side, no new route) + `features.export`; `useTableExport` + `ExportButton` (`export` UI module, `toolbarExtra` slot, CSV/JSON only — Excel unimplemented); `csvExport()` core plugin on the 049 seam + `recordsToCsv`/`recordsToJson`; homepage dogfooded (browser-verified: Export button renders, export path returns the 50k cap over 1M rows); docs (`export.mdx` + ui-and-cli/README); `.changeset/export-ui.md` (core minor, drizzle patch, cli minor). **Deviations from plan-as-written**: (1) added optional `ExportParams.filters`/`sorting`/`maxRows` (plan listed signature change out of scope, but 059's `toolbarExtra` slot surfaces filters/sorting and "export the current view" requires them — additive/backward-compatible); (2) added `httpAdapter.exportData` (not in plan scope, but the marketing dogfood + the dominant client-adapter setup need it; ~20 lines reusing the fetch proxy). |
