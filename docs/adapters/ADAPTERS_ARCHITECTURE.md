...
abstract class BaseAdapter<TData = any> implements TableAdapter<TData> {
  ...
  protected applyFilters(query: QueryBuilder, filters: FilterState[]): QueryBuilder {
    // Common filter application logic
  }
  
  protected applySorting(query: QueryBuilder, sorting: SortingParams[]): QueryBuilder {
    // Common sorting application logic
  }
  
  protected applyPagination(query: QueryBuilder, pagination: PaginationParams): QueryBuilder {
...
import type { TableAdapter, FetchDataParams, FetchDataResult } from '@better-tables/core';
...
export class MemoryAdapter<TData = any> extends BaseAdapter<TData> {
  ...
  private evaluateFilter(value: any, filter: FilterState): boolean {
    ...
    switch (filter.operator) {
      ...
      case 'startsWith':
        return String(value).toLowerCase().startsWith(String(filter.values[0]).toLowerCase());
      case 'endsWith':
        return String(value).toLowerCase().endsWith(String(filter.values[0]).toLowerCase());
      case 'gt':
...
export class RestAdapter<TData = any> extends BaseAdapter<TData> {
  ...
  async getFilterOptions(columnId: string): Promise<FilterOption[]> {
    try {
      const response = await this.makeRequest(`${this.baseUrl}/filter-options/${columnId}`, {
        method: 'GET',
        headers: this.headers,
      });
  ...
  async getFacetedValues(columnId: string): Promise<Map<string, number>> {
    try {
      const response = await this.makeRequest(`${this.baseUrl}/facets/${columnId}`, {
        method: 'GET',
        headers: this.headers,
      });
  ...
  async getMinMaxValues(columnId: string): Promise<[number, number]> {
    try {
      const response = await this.makeRequest(`${this.baseUrl}/minmax/${columnId}`, {
        method: 'GET',
        headers: this.headers,
      });
  ...
  async bulkUpdate(ids: string[], data: Partial<TData>): Promise<TData[]> {
    try {
      const response = await this.makeRequest(`${this.baseUrl}/data/bulk-update`, {
        method: 'PUT',
        headers: {
          ...this.headers,
          'Content-Type': 'application/json',
  ...
  async bulkDelete(ids: string[]): Promise<void> {
    try {
      const response = await this.makeRequest(`${this.baseUrl}/data/bulk-delete`, {
        method: 'DELETE',
        headers: {
          ...this.headers,
          'Content-Type': 'application/json',
  ...
  private applyFilters(query: any, filters: FilterState[]): any {
    const conditions = filters.map(filter => {
      ...
      switch (filter.operator) {
        ...
        case 'notIn':
          return sql`${column} NOT IN (${filter.values.join(',')})`;
        case 'isNull':
  ...
  private applySorting(query: any, sorting: SortingParams[]): any {
    return query.orderBy(
      ...sorting.map(sort => {
        ...
        return sort.direction === 'desc' ? sql`${column} DESC` : sql`${column} ASC`;
      })
    );
  }
}
...
export class CustomAdapter<TData = any> extends BaseAdapter<TData> {
  ...
  private async customGetFilterOptions(columnId: string): Promise<FilterOption[]> {
    // Your custom implementation
  }
}
```
...
class CachedAdapter<TData = any> extends BaseAdapter<TData> {
