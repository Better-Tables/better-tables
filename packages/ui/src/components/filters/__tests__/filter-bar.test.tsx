...
describe('FilterBar', () => {
  ...
  it('should render search input when searchable is true', () => {
    render(<FilterBar {...defaultProps} searchable={true} />);
  ...
  it('should not render search input when searchable is false', () => {
    render(<FilterBar {...defaultProps} searchable={false} />);
  ...
  it('should handle max filters limit', () => {
    const manyFilters: FilterState[] = [
      { columnId: 'name', type: 'text', operator: 'contains', values: ['test'] },
      { columnId: 'age', type: 'number', operator: 'gte', values: [25] },
    ];

    render(<FilterBar {...defaultProps} filters={manyFilters} maxFilters={2} />);
  ...
  it('should render custom filters', () => {
    ...
    render(<FilterBar {...defaultProps} customFilters={[customFilter]} />);
  ...
  it('should handle groups when provided', async () => {
    ...
    render(<FilterBar {...defaultProps} groups={groups} showGroups={true} />);
  ...
  it('should handle custom add filter label', () => {
    render(<FilterBar {...defaultProps} addFilterLabel="Add New Filter" />);

    expect(screen.getByRole('button', { name: /add new filter/i })).toBeInTheDocument();
  });

  it('should handle custom search placeholder', () => {
    render(<FilterBar {...defaultProps} searchable={true} searchPlaceholder="Find columns..." />);
  ...
  it('should handle empty message when no filterable columns', () => {
    ...
    render(<FilterBar columns={nonFilterableColumns} filters={[]} onFiltersChange={vi.fn()} />);
  ...
  it('should handle theme customization', () => {
    ...
    render(<FilterBar {...defaultProps} theme={theme} />);
