...
describe('FilterBar stable handlers (UI-09)', () => {
  ...
  it('updating one filter re-renders only that filter badge, not its sibling', () => {
    ...
    // Strict Mode may double-mount; compare deltas, not absolute counts.
    const aBefore = counts.get('a') ?? 0;
    const bBefore = counts.get('b') ?? 0;
    expect(aBefore).toBeGreaterThan(0);
    expect(bBefore).toBeGreaterThan(0);
    rerender(
      <FilterBar
        columns={cols}
        filters={[{ ...filterA, values: ['z'] }, filterB]}
        onFiltersChange={() => {}}
        showAddFilter={false}
      />
    );
    expect(counts.get('a')).toBeGreaterThan(aBefore);
    expect(counts.get('b')).toBe(bBefore);
  });
});
