...
describe('DrizzleAdapter', () => {
  ...
  describe('Complex Filter Combinations', () => {
    it('should handle multiple text filters with AND logic', async () => {
      const result = await adapter.fetchData({
        filters: [
    ...
    it('should handle multiple number filters with different operators', async () => {
      const result = await adapter.fetchData({
        filters: [
          { columnId: 'age', type: 'number', operator: 'greaterThan', values: [20] },
          { columnId: 'age', type: 'number', operator: 'lessThan', values: [40] },
        ],
      });
      expect(result.data).toHaveLength(3); // All users are between 20-40
    });

    it('should handle date and text filter combinations', async () => {
      const pastDate = Date.now() - 86400000; // Yesterday
      const result = await adapter.fetchData({
        filters: [
          { columnId: 'createdAt', type: 'date', operator: 'after', values: [pastDate] },
          { columnId: 'name', type: 'text', operator: 'isNotNull', values: [] },
        ],
      });
      expect(result.data).toHaveLength(3); // All users created after yesterday
    });
    ...
    it('should handle complex relationship filtering', async () => {
      const result = await adapter.fetchData({
        columns: ['name', 'email'],
        filters: [
          { columnId: 'profile.bio', type: 'text', operator: 'isNotNull', values: [] },
          { columnId: 'posts.title', type: 'text', operator: 'contains', values: ['Post'] },
        ],
      });
      expect(result.data).toHaveLength(1);
      expect((result.data[0] as UserWithRelations).name).toBe('John Doe');
    ...
    it('should handle range filters with text filters', async () => {
      const result = await adapter.fetchData({
        filters: [
          { columnId: 'age', type: 'number', operator: 'between', values: [25, 35] },
          { columnId: 'name', type: 'text', operator: 'notEquals', values: ['Bob Johnson'] },
        ],
      });
      expect(result.data).toHaveLength(2); // John Doe and Jane Smith
    });

    it('should handle multiple relationship filters', async () => {
      const result = await adapter.fetchData({
        columns: ['name', 'email'],
        filters: [
          { columnId: 'profile.bio', type: 'text', operator: 'isNotNull', values: [] },
      ...
      expect(result.data).toHaveLength(2); // John Doe and Jane Smith (both have profiles and published posts)
    });

    it('should handle complex date range with text filters', async () => {
      const now = Date.now();
      const yesterday = now - 86400000;
      const result = await adapter.fetchData({
        filters: [
          { columnId: 'createdAt', type: 'date', operator: 'after', values: [yesterday] },
          { columnId: 'createdAt', type: 'date', operator: 'before', values: [now + 86400000] },
          { columnId: 'name', type: 'text', operator: 'contains', values: ['John'] },
        ],
      });
      expect(result.data).toHaveLength(2); // John Doe and Bob Johnson
    ...
    it('should handle multiple boolean filters', async () => {
      const result = await adapter.fetchData({
        columns: ['name', 'posts.title'],
        filters: [{ columnId: 'posts.published', type: 'boolean', operator: 'isTrue', values: [] }],
      });
      expect(result.data).toHaveLength(2); // John Doe and Jane Smith (both have published posts)
    });

    it('should handle complex nested relationship filtering', async () => {
      // Test filtering users who have profiles with specific bio AND posts with specific titles
      const result = await adapter.fetchData({
        columns: ['name', 'email'],
        filters: [
          { columnId: 'profile.bio', type: 'text', operator: 'contains', values: ['developer'] },
          { columnId: 'posts.title', type: 'text', operator: 'startsWith', values: ['First'] },
        ],
      });
      expect(result.data).toHaveLength(1);
      expect((result.data[0] as UserWithRelations).name).toBe('John Doe');
    ...
    it('should handle edge case with empty filter values', async () => {
      const result = await adapter.fetchData({
        filters: [
          { columnId: 'name', type: 'text', operator: 'isNotNull', values: [] },
      ...
      expect(result.data).toHaveLength(3); // All users have names and ages
    });

    it('should handle complex case-insensitive text filtering', async () => {
      const result = await adapter.fetchData({
        filters: [
          { columnId: 'name', type: 'text', operator: 'contains', values: ['JOHN'] }, // uppercase
