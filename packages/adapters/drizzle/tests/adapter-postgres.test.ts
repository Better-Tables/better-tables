  ...
  () => {
    ...
    describe('Text Filter Operators', () => {
      ...
      it('should fail-soft for text notEquals (not in core TEXT_OPERATORS, plan 038)', async () => {
        ...
        // advertises `notEquals`. An unsupported operator is fail-soft (filter
        // dropped), so the query still returns rows including 'John Doe'.
        const result = await adapter.fetchData({
          filters: [
            ...
            users: [
              {
                ...
                compute: (row: Record<string, unknown>) => {
                  ...
                  return ageValue !== null && ageValue !== undefined
                    ? `Age: ${ageValue}`
                    : 'Unknown';
                },
              },
    ...
    describe('Array Column Filtering', () => {
      ...
      it('should filter by isAnyOf operator on uuid[] column', async () => {
        ...
        const eventsDb = drizzlePostgres<typeof eventsSchema>(eventsClient, {
          schema: eventsSchema,
        });
    ...
    describe('Computed Fields', () => {
      ...
      it('should handle computed field filtering', async () => {
        ...
        const result = await adapterWithComputed.fetchData({
          columns: ['name', 'postCount'],
          filters: [
            { columnId: 'postCount', operator: 'greaterThan', values: [0], type: 'number' },
          ],
        });
